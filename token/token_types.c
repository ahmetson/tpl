...
int is_token_loop_sttmnt(token *tok, char *tok_val)
{
	...
	for(i=0; i<TOK_CLASS_LOOP_STTMNT_NUM; i++)
	{
		...
		answer = strstr_by_offset(TOK_CLASS_LOOP_STTMNT_CHARS[i][0], tok_val, 0);
		if (answer>=0)
		{
			tok_type.type_num = i;							// Number of token type
...
int is_token_void(token *tok, char *tok_val)
{
	...
	answer = strstr_by_offset(TOK_VOID_CHARS[0][0], tok_val, 0);
    if (answer>=0)
    {
        tok_type.type_num = 0;							// Number of token type
