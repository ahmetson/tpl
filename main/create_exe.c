...
void create_exe()
{
    ...
    /// Make Object files from C source codes
    int i, line_len;
    ...
    wchar_t std_chap_et[ MAX_FILE_LEN ] = { 0 };
    wcsncpys( std_chap_et, DIR_C_FOLDER );
    wcscat( std_chap_et, L"_tpl_std_chap_et.c" );
    {
        line_len = ( wcslen( FILE_GCC_EXE ) + wcslen( GCC_C_ARG )*2 + wcslen( std_chap_et )*2 + wcslen( GCC_C_TO_O_ADDS ) + 5 )*sizeof( wchar_t );
        line = malloc( line_len );
        wcsncpys( line, FILE_GCC_EXE );
        ...
        // Add .O file source
        wchar_t obj_source[ MAX_FILE_LEN ] = { 0 };
        wcsncpys( obj_source, std_chap_et );
        obj_source[ wcslen( obj_source )-1 ] = L'o';              // change .c extension to .o extension
        wcsncat( line, obj_source, wcslen( obj_source ) );
    ...
    /// Link Object files and make one executable file
    line_len = ( wcslen( FILE_GPP_EXE ) + wcslen( OBJ_LIST ) + wcslen( GCC_O_ARG ) + wcslen( FILE_OUT_EXE ) + wcslen( GCC_O_TO_EXE_ADDS ) + 3)*sizeof( wchar_t );
    line = malloc( line_len );

    wcsncpys( line, FILE_GPP_EXE );
