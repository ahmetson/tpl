...
int is_cmd_call_glob_var(command *cmd)
{
    ...
    if (cmd->items_num>1 && cmd->items[1].type==TOKEN_ITEM)
    {
...
int semantic_cmd_call_glob_var(command *cmd)
{
    ...
    token *item = &cmd->items[1].tok; // Gysgaltmak uchin ulanlyar
...
int cmd_call_glob_var_return_type(command *cmd, int *return_class, int *ret_type)
{
    ...
    **/
    if (is_glob_var_def_exist(cmd->items[1].tok.potentional_types[0].value))
    {
        get_var_def_value_type(cmd->items[1].tok.potentional_types[0].value, return_class, ret_type, 1);
        return 1;
    }
    else if (is_glob_var_dec_exist(cmd->items[1].tok.potentional_types[0].value))
    {
        get_glob_var_dec_value_type(cmd->items[1].tok.potentional_types[0].value, return_class, ret_type);
        return 1;
    }
    CUR_PART = 7;
...
void cmd_call_glob_var_c_code(command *cmd, char **line, int *llen)
{
    ...
    token_type *ty = &cmd->items[1].tok.potentional_types[0];
